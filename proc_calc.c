...
ssize_t write_not_permitted(struct file *filp,
...
ssize_t my_write_op1(struct file* filp, const char __user* buf,
...
ssize_t my_write_op2(struct file* filp, const char __user* buf,
		     size_t len, loff_t* offset)
{
	printk(KERN_INFO "Entering function %s\n", __FUNCTION__ );
	if (len < 0)
...
ssize_t my_write_operand(struct file* filp, const char __user* buf,
			 size_t len, loff_t* offset)
{
	int i = 0;
...
ssize_t read_not_permitted(struct file *filp, char __user *buffer,
